...
// 設計適当ですが許して
// 第一引数：解析場の情報を保存，第二引数：正の値で終了ステップを入力
void FDMSolverForDiffusionEq::SolveHeatEquationForAllStep(vtkUnstructuredGrid *out_field, const int &in_endStep)
{
...
bool FDMSolverForDiffusionEq::SolveHeatEquationForOneStep(vtkUnstructuredGrid *out_field)
{
  // 一つ前の結果を取り出す
...
void FDMSolverForDiffusionEq::SetParameters(vtkUnstructuredGrid *out_field, const double &in_alpha)
{
  // out_field の各方向の大きさ
...
// 周辺6個のボクセル中の値の平均をとる
void FDMSolverForDiffusionEq::SetAveValueFromSurroundings(const vtkIdType in_voxelId, vtkDoubleArray *out_results, vtkDataArray *in_properties)
{
  ...
  for (vtkIdType pxyzId = 0; pxyzId < 6; pxyzId++)
  {
    ...
    if (vId < 0 || out_results->GetNumberOfTuples() <= vId) continue;
    if (in_properties->GetTuple1(vId) != Analysis) continue;
...
void FDMSolverForDiffusionEq::SetBoundryCondition(vtkDoubleArray * in_results, vtkDataArray * in_properties)
{
  // 全ボクセル参照
